...
def switching_boards(proxy) -> list:
    """
    Parameters
    ----------
    ...
    """
    return [hv.HVSwitchingBoard(proxy, address=address) for address in proxy.i2c_scan() if address in [32, 33, 34]]


def switching_boards_info(switching_boards) -> pd.DataFrame:
    """
    Parameters
    ----------
    switching_boards
    ...
    switching_boards_info = pd.DataFrame(({k: getattr(switching_board_i, k)().split(b'\0')[0]
                                         ...
                                         index=pd.Series([switching_board_i.address
                                                          for switching_board_i in switching_boards],
                                                         name='address'))
    switching_boards_info['uuid'] = [switching_board_i.uuid for switching_board_i in switching_boards]
